...
do
    if (useCustomErrorChecker) then
        ...
        local character2 = stats.defaultb
        ...
        checkName(character2.name, false)

        checkCostume(character2.costume)

        checkItems(character2.items)

        checkGeneric(character2.trinket, "trinket", -1)

        checkGeneric(character2.card, "card", -1)

        checkGeneric(character2.pill, "pill", -1)

        checkGeneric(character2.charge, "charge")
        ...
        if (stats.tainted.enabled) then
            ...
            checkName(character2.name, true)
        ...
        if (stats.defaultb.name == "Alpha" or (stats.taintedb.name == "Omega" and stats.taintedb.enabled)) then
            errorChecker.printError("You must change the character(s) name from the default!")
            errorChecker.printError("Name: " .. stats.defaultb.name)
            if (stats.taintedb.enabled) then
                errorChecker.printError("Name: " .. stats.taintedb.name)
            end
        end
...
local char2 = Isaac.GetPlayerTypeByName(stats.defaultb.name, false)
local taintedChar = Isaac.GetPlayerTypeByName(stats.tainted.name, true)
taintedChar = taintedChar == -1 and char or taintedChar
local taintedChar2 = Isaac.GetPlayerTypeByName(stats.taintedb.name, true)
taintedChar2 = taintedChar2 == -1 and char2 or taintedChar2

-- Utility Functions
local function IsChar(player)
    if (player == nil) then return nil end
    local ptype = player:GetPlayerType()
    if (ptype ~= char and ptype ~= taintedChar and ptype ~= char2 and ptype ~= taintedChar2) then return false end
    return true
end
...
local function IsTainted(player)
    if (player == nil) then return nil end
    local ptype = player:GetPlayerType()
    if (ptype ~= char and ptype ~= taintedChar and ptype ~= char2 and ptype ~= taintedChar2) then return nil end
    if (ptype == char or ptype == char2) then return false end
    return true
end
...
local function GetPlayers()
    ...
    for i = 0, game:GetNumPlayers() - 1 do
        ...
        local ptype = player:GetPlayerType()
        if (ptype == char or ptype == taintedChar or ptype == char2 or ptype == taintedChar2) then
...
local function GetPlayerStatTable(player)
    local ptype = player:GetPlayerType()
    if (ptype == char) then
        return stats.default end
    if (ptype == char2) then
        return stats.defaultb end
    if (ptype == tainted) then
        return (taint and stats.tainted) end
    if (ptype == taintedb) then
        return (taint and stats.taintedb) end
    --[[
...
--[[%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            External Item Description array setup
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%]]

if not __eidItemDescriptions then __eidItemDescriptions = {} end
if not __eidTrinketDescriptions then __eidTrinketDescriptions = {} end
if not __eidCardDescriptions then __eidCardDescriptions = {} end
if not __eidItemTransformations then __eidItemTransformations = {} end


--[[%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            Tear Animation Scaling & Setup (modeled from better animated Tears)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%]]
