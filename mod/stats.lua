...
setmetatable(stats.defaultb, mt)
setmetatable(stats.taintedb, mt)

local character = stats.default
local tainted = stats.tainted

local characterb = stats.defaultb
local taintedb = stats.taintedb
...
character.name = "Craig" -- The name of your character (must match the players.xml file).

character.stats = {
    damage = 0.40, -- float
    firedelay = - 1.00, -- float
    shotspeed = -0.60, -- float
    range = 3.00, -- float
    speed = 0.30, -- float
    tearflags = TearFlags.TEAR_PERSISTENT, -- Enum "TearFlags"
...
character.costume = "character_craig_hair"
--[[
...
character:AddItem(Isaac.GetItemIdByName("Lvl 9"), true) 
character:AddItem(Isaac.GetItemIdByName("B Button"), true) -- I want Sad Onion and give me the costume.
--character:AddItem(CollectibleType.COLLECTIBLE_CHAMPION_BELT, true)
...
character.trinket = Isaac.GetTrinketIdByName("GK Controller")
--[[
...
character.card = Card.CARD_STARS
--[[
...
tainted.stats = {
    ...
    tearcolor = Color(0, 0, 1.0, 1.0, 0, 0, 0),
    flying = false,
    luck = 1.00
}
...
--[[
	PORTER REGULAR CHARACTER SETUP
]]
characterb.name = "Porter" -- The name of your character (must match the players.xml file).
...
characterb.stats = {
    ...
    range = 3.00, -- float
    speed = - 0.2, -- float
    tearflags = TearFlags.TEAR_NORMAL, -- Enum "TearFlags"
    tearcolor = Color(1, 1, 1, 1, 0, 0, 0), -- Color, first four arguments between 0 and 1
    flying = false, -- boolean
    luck = 1.00 -- float
}

characterb.costume = "character_porter_hair"

characterb:AddItem(Isaac.GetItemIdByName("Position 1"), true)


characterb.trinket = TrinketType.TRINKET_NULL

characterb.card = Card.CARD_NULL

characterb.pill = false

characterb.charge = -1

--[[
	TAINTED PORTER CHARACTER SETUP
]]
taintedb.enabled = true -- set this to false if you don't want a tainted character.

taintedb.name = "Bastion"
taintedb.stats = {
    damage = 2.00,
    firedelay = 1.00,
    shotspeed = 1.00,
...
taintedb.trinket = TrinketType.TRINKET_SWALLOWED_PENNY

taintedb.card = Card.CARD_FOOL

taintedb.pill = false

taintedb.charge = -1

taintedb:AddItem(CollectibleType.COLLECTIBLE_SAD_ONION) -- I want Sad Onion and give me the costume.
